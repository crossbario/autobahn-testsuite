/*
 AutobahnJS - https://autobahn-testsuite.readthedocs.io/
...
p[0]++}i.sigBytes=4*n;return i}});a.PBKDF2=function(a,c,b){return f.create(b).compute(a,c)}})();/*
 MIT License (c) Copyright (c) Crossbar.io Technologies GmbH. */
...
ab.getServerUrl=function(a,b){return"file:"===window.location.protocol?b?b:"ws://127.0.0.1/ws":("https:"===window.location.protocol?"wss://":"ws://")+window.location.hostname+(""!==window.location.port?":"+window.location.port:"")+"/"+(a?a:"ws")};ab.browserNotSupportedMessage="Browser does not support WebSocket (RFC6455)";
...
a,b,e];else throw"invalid argument type(s)";else f=[ab._MESSAGE_TYPEID_PUBLISH,a,b];ab._debugpubsub&&(console.group("WAMP Publish"),console.info(this._wsuri+"  ["+this._session_id+"]"),console.log(a),console.log(b),null!==d?console.log(d):null!==e&&(console.log(e),null!==c&&console.log(c)),console.groupEnd());this._send(f)};ab.Session.prototype.authreq=function(a,b){return this.call("http://api.wamp-proto.org/procedure#authreq",a,b)};
ab.Session.prototype.authsign=function(a,b){b||(b="");return CryptoJS.HmacSHA256(a,b).toString(CryptoJS.enc.Base64)};ab.Session.prototype.auth=function(a){return this.call("http://api.wamp-proto.org/procedure#auth",a)};
